...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside 
      className={`bg-slate-800 shadow-xl flex flex-col custom-scroll transition-all duration-300 ease-in-out
    ...
    >
      ...
      {!isMobile && (
        <button 
          onClick={toggleSidebar} 
          className="absolute top-1/2 -right-3 transform -translate-y-1/2 w-6 h-16 bg-slate-700 hover:bg-slate-600 text-white rounded-r-md flex items-center justify-center focus:outline-none"
          title={isSidebarOpen ? "Collapse Sidebar" : "Expand Sidebar"}
          aria-label={isSidebarOpen ? "Collapse Sidebar" : "Expand Sidebar"}
          style={{ display: isSidebarOpen ? 'flex' : 'flex' }} // Always display toggle button for desktop
        >
